do --// UI Source
    ...
    Library = {
        ...
        local CustomFont = { } do
            ...
            Library.Font = CustomFont:New("ProggyClean", 400, "normal", {
                Id = "ProggyClean.ttf",
        ...
        do
            ...
            Library.Slider = function(Self, Params)
                ...
                function Slider:AddSlider(Params)
                    Params = Params or { }
                    ...
                    local SubItems = { } do
                        -- Use the same parent frame
                        SubItems["Slider"] = Items["Slider"]

                        SubItems["RealSlider"] = Library:Create("TextButton", {
                            Name = "\0",
                            FontFace = Library.Font,
                            TextSize = Library.FontSize,
                            Text = "",
                            Parent = SubItems["Slider"].Instance,
                            TextColor3 = Color3.fromRGB(0, 0, 0),
                            AutoButtonColor = false,
                            AnchorPoint = Vector2.new(0, 1),
                            Position = UDim2.new(0, 2, 1, 0),
                        ...
                        Library:Create("UIStroke", {
                            Name = "\0",
                            Parent = SubItems["RealSlider"].Instance,
                            ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                            LineJoinMode = Enum.LineJoinMode.Miter,
                            Color = Library.Theme["Outline 3"],
                            BorderOffset = UDim.new(0, 1)
                        ...
                        SubItems["Accent"] = Library:Create("Frame", {
                            Name = "\0",
                            Parent = SubItems["RealSlider"].Instance,
                            Size = UDim2.new(0.5, 0, 1, 0),
                            BorderSizePixel = 0,
                        ...
                        SubItems["Value"] = Library:Create("TextLabel", {
                            Name = "\0",
                            FontFace = Library.Font,
                            TextSize = Library.FontSize,
                            Parent = SubItems["Accent"].Instance,
                            TextColor3 = Library.Theme["Text"],
                            Text = "0",
                            AnchorPoint = Vector2.new(1, 0.5),
                            Size = UDim2.new(0, 0, 0, 12),
                        ...
                        Library:Create("UIStroke", {
                            Name = "\0",
                            Parent = SubItems["Value"].Instance
                        })
                        ...
                        SubItems["RealSlider"]:OnHover(function()
                            SubItems["RealSlider"]:Tween({BackgroundColor3 = Library.Theme["Hovered Element"]})
                        end, function()
                            SubItems["RealSlider"]:Tween({BackgroundColor3 = Library.Theme["Content"]})
                        end)

                        SubSlider.Items = SubItems
                    end

                    -- Horizontally layout all chained sliders
                    if not Slider.RowSliders then
                        Slider.RowSliders = { Slider }
                    end
                    table.insert(Slider.RowSliders, SubSlider)
                    SubSlider.RowSliders = Slider.RowSliders

                    local Count = #Slider.RowSliders
                    for i, Sldr in ipairs(Slider.RowSliders) do
                        local ScaleX = 1 / Count
                        local OffX = (i - 1) * ScaleX
                        
                        Sldr.Items["RealSlider"].Instance.Size = UDim2.new(ScaleX, -4, 0, 3)
                        Sldr.Items["RealSlider"].Instance.Position = UDim2.new(OffX, 2, 1, 0)
                        Sldr.Items["Text"].Instance.Position = UDim2.new(OffX, 1, 0, 0)
                    end

                    function SubSlider:Set(Value)
                        SubSlider.Value = Library:Round(math.clamp(Value, SubSlider.Min, SubSlider.Max), SubSlider.Decimals)

                        SubItems["Accent"]:Tween({Size = UDim2.new((SubSlider.Value - SubSlider.Min) / (SubSlider.Max - SubSlider.Min), 0, 1, 0)}, TweenInfo.new(Library.Animation.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out))
                        SubItems["Value"].Instance.Text = string.format("%s%s", SubSlider.Value, SubSlider.Suffix)

                        Flags[SubSlider.Flag] = SubSlider.Value
                        Library:SafeCall(SubSlider.Callback, SubSlider.Value)
                    end

                    function SubSlider:GetSize(Input)
                        local SizeX = (Input.Position.X - SubItems["RealSlider"].Instance.AbsolutePosition.X) / SubItems["RealSlider"].Instance.AbsoluteSize.X
                        local Value = ((SubSlider.Max - SubSlider.Min) * SizeX) + SubSlider.Min
                        return Value
                    end
                    ...
                    local SubInputChanged

                    SubItems["RealSlider"]:Connect("InputBegan", function(Input)
                        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                            SubSlider.Sliding = true

                            local Value = SubSlider:GetSize(Input)
                            SubSlider:Set(Value)

                            if SubInputChanged then
                                return
                            end

                            SubInputChanged = Input.Changed:Connect(function()
                                if Input.UserInputState == Enum.UserInputState.End then
                                    SubSlider.Sliding = false
                                    SubInputChanged:Disconnect()
                                    SubInputChanged = nil
                                end
                            end)
                        end
                    ...
                    SubSlider:Set(SubSlider.Default)

                    if SubSlider.Flag and SubSlider.Flag ~= "" then
                        SetFlags[SubSlider.Flag] = function(Value)
                            SubSlider:Set(Value)
                        end
                    end

                    return setmetatable(SubSlider, Library)
                end
